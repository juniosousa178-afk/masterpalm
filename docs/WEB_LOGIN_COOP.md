...
1. **Header na sua URL**  
   Abra **https://app.mastepalm.com.br/** (domínio canônico do app Web; se ainda houver tráfego no domínio compat, teste também `https://app.masterpalm.com.br/`). DevTools → **Network** → recarregue → clique na primeira requisição (documento HTML). Em **Response Headers** deve aparecer:
   ```http
   Cross-Origin-Opener-Policy: same-origin-allow-popups
   ```
   Se você usa domínio customizado (canônico: **app.mastepalm.com.br**; compat: app.masterpalm.com.br) e um CDN/proxy na frente do Firebase Hosting, esse header precisa ser repassado (ou configurado no CDN).
