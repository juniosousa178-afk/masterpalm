...
const _demoProjectId = 'demo-masterpalm-sale-intent-m32c';

/// M3.2-C — order review + Sale Intent no Emulator (OR-EM-1/2/3).
void main() {
  group('Sale intent MD2-C — order review emulator', () {
    ...
    test(
      'executa runner Node MD2-C order review',
      () async {
        final host = Platform.environment['FIRESTORE_EMULATOR_HOST'];
        final reason = _abortReason(host);
        if (reason != null) {
        ...
        final runnerPath =
            '$repoRoot/test/sale_intent/support/sale_intent_md2c_emulator_runner.mjs'
                .replaceAll('\\', '/');
        ...
        expect(
          result.exitCode,
          0,
          reason: 'Runner MD2-C falhou (exit=${result.exitCode}). '
