...
const _demoProjectId = 'demo-masterpalm-fiado-040b2';

/// 040B2/040B3 — rules de ContaReceber (PAID→OPEN DENY + duplicate guard).
void main() {
  group('FIADO 040B2 — contas_receber rules emulator', () {
    ...
    test(
      'executa runner Node 040B2 contas_receber',
      () async {
        final host = Platform.environment['FIRESTORE_EMULATOR_HOST'];
        final reason = _abortReason(host);
        if (reason != null) {
        ...
        final runnerPath =
            '$repoRoot/tools/fiado_040b2_cr_rules_runner.mjs'
                .replaceAll('\\', '/');
        ...
        expect(
          result.exitCode,
          0,
          reason: 'Runner 040B2 rules falhou (exit=${result.exitCode}). '
