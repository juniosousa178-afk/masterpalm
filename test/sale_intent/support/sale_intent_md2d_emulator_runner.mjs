/**
 * M3.2-D — pedido público + Sale Intent no Emulator (PP-EM-1..5).
 * projectId: demo-masterpalm-sale-intent-m32d
 */
...
const PROJECT_ID = "demo-masterpalm-sale-intent-m32d";
const LOJA = "loja-md2d-pre-pedido";
const OWNER_UID = "owner_md2d_pp";
...
async function main() {
  console.log(`MD2-D pre_pedido | project=${PROJECT_ID} | host=${emulatorHost}`);
  ...
  try {
    ...
    console.log("PP-EM-1 — mesmo pedido → mesmo operationId");
    ...
    console.log("PP-EM-2 — hash divergente fail-closed");
    ...
    await check("PP-EM-2 conflito hash", async () => {
      if (!em2Conflict) throw new Error("esperava conflito");
    });

    console.log("PP-EM-3 — pedidos diferentes → operationIds distintos");
    ...
    await check("PP-EM-3 operationIds distintos", async () => {
      if (rA.operationId === rB.operationId) {
        throw new Error("operationIds colapsados");
      }
    });

    console.log("PP-EM-4 — retry reverted → reserved join");
    ...
    await check("PP-EM-4 reverted→reserved join", async () => {
    ...
    console.log("PP-EM-5 — completed join preserva operationId");
    ...
    await check("PP-EM-5 completed join", async () => {
