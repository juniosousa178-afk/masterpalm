...
const _demoProjectId = 'demo-masterpalm-cat-side-rules';

/// M3.7.1 — RULESEC catalogo_side_effects_secundarios no Emulator local.
void main() {
  group('M3.7.1 RULESEC — catalogo_side_effects_secundarios rules emulator', () {
    ...
    test(
      'executa runner RULESEC com Client SDK autenticado no Emulator',
      () async {
        final host = Platform.environment['FIRESTORE_EMULATOR_HOST'];
        final reason = _abortReason(host);
        if (reason != null) {
        ...
        final runnerPath =
            '$repoRoot/test/catalogo_side_effects_secundarios_rules/support/catalogo_side_effects_secundarios_rules_runner.mjs'
                .replaceAll('\\', '/');
        ...
        expect(
          result.exitCode,
          0,
          reason: 'Runner RULESEC falhou (exit=${result.exitCode}). '
