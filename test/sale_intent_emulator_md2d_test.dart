...
const _demoProjectId = 'demo-masterpalm-sale-intent-m32d';

/// M3.2-D — pedido público + Sale Intent no Emulator (PP-EM-1..5).
void main() {
  group('Sale intent MD2-D — pre_pedido emulator', () {
    ...
    test(
      'executa runner Node MD2-D pre_pedido',
      () async {
        final host = Platform.environment['FIRESTORE_EMULATOR_HOST'];
        final reason = _abortReason(host);
        if (reason != null) {
        ...
        final runnerPath =
            '$repoRoot/test/sale_intent/support/sale_intent_md2d_emulator_runner.mjs'
                .replaceAll('\\', '/');
        ...
        expect(
          result.exitCode,
          0,
          reason: 'Runner MD2-D falhou (exit=${result.exitCode}). '
