...
const _demoProjectId = 'demo-masterpalm-sale-intent-m32b';

/// M3.2-B — integração PDV coordenado no Emulator (MD2-B-T1/T2/T3).
void main() {
  group('Sale intent MD2-B — emulator', () {
    ...
    test(
      'executa runner Node MD2-B com coordenação + baixa idempotente',
      () async {
        final host = Platform.environment['FIRESTORE_EMULATOR_HOST'];
        final reason = _abortReason(host);
        if (reason != null) {
        ...
        final runnerPath =
            '$repoRoot/test/sale_intent/support/sale_intent_md2b_emulator_runner.mjs'
                .replaceAll('\\', '/');
        ...
        expect(
          result.exitCode,
          0,
          reason: 'Runner MD2-B falhou (exit=${result.exitCode}). '
