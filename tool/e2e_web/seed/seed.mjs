/**
 * Seed sintético Firestore + Auth Emulator — R8.4.39
 * Segunda execução restaura estado inicial (sobrescreve documentos canônicos).
 */
import {
  AUTH_HOST,
  CLIENTE_ID,
...
async function main() {
  ...
  await putDoc(`lojas/${LOJA_ID}/estoque_produtos/${PROD_VAR_ID}`, produtoBase(PROD_VAR_NOME, 18, {
    ...
    estoquePorTamanho: {
      mapValue: {
        fields: { P: int(5), M: int(6), G: int(7) },
      },
    },
    variacoes: {
      mapValue: {
    ...
    tamanhos: {
      arrayValue: { values: [str('P'), str('M'), str('G')] },
    },
  }));
  ...
  await putDoc(`lojas/${LOJA_ID}/compras_fornecedor/${PEDIDO_REVENDA_VAZIO_ID}`, {
    ...
    fornecedorNome: str('Fornecedor QA R8439'),
  ...
  await putDoc(`lojas/${LOJA_ID}/compras_fornecedor/${PEDIDO_REVENDA_CONTRACT_ID}`, {
    tipoCompra: str('revenda_detalhar_depois'),
