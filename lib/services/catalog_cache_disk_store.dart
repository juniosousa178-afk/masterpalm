...
String _cfgKey(String lojaId, bool preview) =>
    'cfg:${lojaId}_$preview';
String _metaKey(String lojaId, bool preview) =>
    'meta:${lojaId}_$preview';
...
({Map<String, dynamic> cfg, int removed}) _sanitizeConfigForDisk(
  ...
  return (cfg: r.value as Map<String, dynamic>, removed: r.removed);
}
...
dynamic _toJsonSafe(dynamic value) {
  ...
  if (value is Map) {
    return value.map<String, dynamic>(
        (k, v) => MapEntry(k.toString(), _toJsonSafe(v)));
  }
  if (value is List) {
...
class CatalogCacheDiskStore {
    ...
    try {
      ...
      if (kEnableCatalogDiskCacheAuditLogs) {
        ...
        if (kEnableCatalogDiskCacheSanitize && removedCount > 0) {
          logD('[CACHE-DISK] write sanitized: removed $removedCount keys', tag: 'CACHE-DISK');
        }
      }
    } catch (e) {
      logW('[CACHE-DISK] writeConfig falhou (type=${e.runtimeType})', tag: 'CACHE-DISK');
    }
  }
  ...
  Future<({Map<String, dynamic>? cfg, int? updatedAtMs})> readConfig(
    ...
    try {
      ...
      if (jsonStr == null || jsonStr is! String) {
        if (kEnableCatalogDiskCacheAuditLogs) {
          logD('[CACHE-DISK] read miss preview=$preview temCfg=false', tag: 'CACHE-DISK');
        }
      ...
      return (cfg: cfg, updatedAtMs: updatedAtMs);
    } catch (e) {
      logW('[CACHE-DISK] readConfig falhou (type=${e.runtimeType})', tag: 'CACHE-DISK');
      return (cfg: null, updatedAtMs: null);
    }
  }
  ...
  Future<void> clear(String lojaId, {bool preview = false}) async {
    try {
      ...
      logW('[CACHE-DISK] clear falhou (type=${e.runtimeType})', tag: 'CACHE-DISK');
    }
  }
}
