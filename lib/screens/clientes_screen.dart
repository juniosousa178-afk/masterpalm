...
import '../services/pedido_venda_pdf_service.dart';
...
class _ClientesScreenState extends State<ClientesScreen>
  ...
  Future<void> _init() async {
    ...
    try {
      ...
      logE('[CLIENTES] migração de itens falhou (type=${e.runtimeType})', error: e, st: st);
    }
  ...
  String _resumoProdutosVenda(Venda v) {
    final itens = MigracaoVendasItensService.resolverItens(v);
    if (itens.isNotEmpty) {
  ...
  Future<void> _imprimirPedido(Venda venda) async {
    try {
      ...
      await PedidoVendaPdfService.imprimir(
        venda: venda,
        lojaId: lojaId ?? '',
        cliente: cliente,
      );
