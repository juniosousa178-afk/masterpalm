...
if (process.env.FIRESTORE_EMULATOR_HOST !== '127.0.0.1:8187') throw new Error('Local emulator required; refusing any other endpoint');
...
test('missing data/capability and client-supplied stock/revision are rejected', async () => {
  ...
  // NO_CONTROL without store membership doc → permission-denied (not silent allow).
...
test('sale then editorial preserves stockOperationId lineage', async () => {
  const {base, lojaId} = await seed({quantidade: 2});
  await executeStockCommand(db, intent(lojaId, 'sale', 'sale_Y', {quantity: 1}), owner);
  const afterSale = await getStock(base, 'p');
  assert.equal(afterSale.quantidade, 1);
  assert.equal(afterSale.stockOperationId, 'sale_Y');
  const rev = afterSale.stockRevision;
  await executeStockCommand(db, intent(lojaId, 'editorial', 'f4b68e16', {}, {editorial: {descricao: 'meta'}}), owner);
  const afterEd = await getStock(base, 'p');
  assert.equal(afterEd.quantidade, 1);
  assert.equal(afterEd.stockRevision, rev);
  assert.equal(afterEd.stockOperationId, 'sale_Y');
  assert.equal(afterEd.descricao, 'meta');
});
test('editorial then sale stamps sale operationId', async () => {
  const {base, lojaId} = await seed({quantidade: 2});
  await executeStockCommand(db, intent(lojaId, 'editorial', 'ed_first', {}, {editorial: {descricao: 'x'}}), owner);
  await executeStockCommand(db, intent(lojaId, 'sale', 'sale_after', {quantity: 1}), owner);
  const s = await getStock(base, 'p');
  assert.equal(s.quantidade, 1);
  assert.equal(s.stockOperationId, 'sale_after');
});
test('restore rebound uses sale.stockOperationId when source id missing', async () => {
  const {base, lojaId} = await seed();
  await executeStockCommand(db, intent(lojaId, 'sale', 'real_op', {quantity: 1}), owner);
  assert.equal((await getStock(base, 'p')).quantidade, 0);
  // Simulate modern sale doc whose id != stock op id but binds explicitly.
  await base.collection('estoque_vendas').doc('sale_doc_only').set({
    id: 'sale_doc_only',
    stockOperationId: 'real_op',
    status: 'concluida',
  });
  const restore = intent(lojaId, 'restore', 'restore_1', {}, {sourceOperationId: 'sale_doc_only'});
  await executeStockCommand(db, restore, owner);
  const s = await getStock(base, 'p');
  assert.equal(s.quantidade, 1);
  const op = await base.collection('stock_catalog_operations').doc('real_op').get();
  assert.equal(op.data().restoredBy, 'restore_1');
});
test('restore fail-closed when binding missing and op absent', async () => {
  const {base, lojaId} = await seed();
  await base.collection('estoque_vendas').doc('orphan_sale').set({id: 'orphan_sale', status: 'concluida'});
  await denied(
    executeStockCommand(db, intent(lojaId, 'restore', 'r_fail', {}, {sourceOperationId: 'orphan_sale'}), owner),
    'failed-precondition',
  );
});
