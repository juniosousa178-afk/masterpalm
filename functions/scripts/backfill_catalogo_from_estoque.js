...
Credenciais (obrigatório configurar um):
  --credentials <ficheiro.json>   Chave da conta de serviço (recomendado no Windows)
...
Opções:
  ...
  --remove-legacy-slug   Remove doc duplicado por slug legado (use --dry-run antes)
  --limit N              Processa no máximo N produtos de estoque
...
async function main() {
  ...
  if (lojaPlaceholders.has(args.lojaId.trim().toUpperCase())) {
    console.error(
      `[erro] --lojaId parece placeholder. Use o id real da loja (ex.: o slug em lojas/... no Firestore).`,
    );
    process.exitCode = 1;
    return;
  }
  ...
  console.log(`[backfill] draft=${args.draft} live=${args.live} removeLegacy=${args.removeLegacySlug}`);
  ...
  if (snap.empty) {
    console.log("[backfill] Nenhum documento em estoque_produtos.");
    return;
  }
  ...
  for (const doc of snap.docs) {
    ...
    if (args.draft) {
      writes.push({ ref: draftRef.doc(estoqueId), data: { ...patch }, merge: true });
      console.log(`[draft] merge ${estoqueId} peso=${patch.peso} preco=${patch.preco}`);
    }
    ...
    if (args.live) {
      if (shouldWriteLive(est)) {
        ...
        writes.push({ ref: liveRef.doc(estoqueId), data: livePatch, merge: true });
        console.log(`[live]  merge ${estoqueId} peso=${patch.peso} preco=${patch.preco}`);
      } else {
        skippedLive += 1;
      }
    }
    ...
    if (
      ...
      for (const col of [draftRef, liveRef]) {
        ...
        if (sameProduto) {
          legacyMarked += 1;
          console.log(
            `[legacy] remover duplicata ${col.id}/${slug} (canônico=${estoqueId})`,
          );
          deletes.push(legRef);
        }
      }
  ...
  if (deletes.length > 0 && !args.dryRun) {
    for (let i = 0; i < deletes.length; i += BATCH_MAX) {
      ...
      await batch.commit();
      console.log(`[legacy] delete batch ${slice.length}`);
    }
  } else if (deletes.length > 0 && args.dryRun) {
    console.log(`[legacy] dry-run: ${deletes.length} exclusões NÃO executadas`);
  }

  console.log("");
  console.log("[backfill] Resumo");
  console.log(`  estoque_docs_processados: ${processed}`);
  console.log(`  escritas_agendadas: ${writes.length}`);
  console.log(`  live_ignorados (não publicados/sem estoque): ${skippedLive}`);
  console.log(`  legacy_delete_marcados: ${deletes.length}`);
}
